...
export async function generateMetadata({
  ...
  const property = await prisma.property.findUnique({
    where: { id },
    select: { name: true, description: true, images: true, location: true },
  });
  ...
  return {
    title: property.name,
    description: `Check out this amazing property in ${property.location}. ${property.description?.substring(0, 150)}...`,
    openGraph: {
      title: property.name,
...
export default async function PropertyDetailsPage({
  ...
  const remainingTotalPrice = vacantRooms.reduce(
              ...
              <CardContent className="p-2 md:px-6 md:pb-6">
                ...
                {vacantRooms.length > 1 && (
                  <div className="mt-6 pt-6 border-t text-center">
                    <p className="text-muted-foreground">
                      Or rent all available space
                    </p>
                    <p className="text-2xl font-bold text-primary">
                      {formatCurrency(remainingTotalPrice)}/month
                    </p>
                    <Button className="w-full mt-2">
                      Inquire About Full Rental
                    </Button>
                  </div>
                )}
              </CardContent>

...
export default async function ContactPage({ params}: { params: { locale: string } }) {
 ...
 const contactItems = [
  { icon: MapPin, color: "text-orange-500", bg: "from-orange-100 to-orange-200", labelKey: "addressLabel", valueKey: "address" },
  { icon: Phone,  color: "text-green-500",  bg: "from-green-100 to-green-200",  labelKey: "phoneLabel",   valueKey: "phone1", secondaryValueKey: "phone2" },
  { icon: Mail,   color: "text-blue-500",   bg: "from-blue-100 to-blue-200",   labelKey: "emailLabel",   valueKey: "email" },
  { icon: Clock,  color: "text-yellow-500", bg: "from-yellow-100 to-yellow-200", labelKey: "hoursLabel", valueKey: "hours1", secondaryValueKey: "hours2" },
];
    ...
    return (
        <div className="-mt-17">
            ...
            <section className="relative  py-30 p-2 text-center md:text-start  pb-15 overflow-hidden bg-gradient-to-br from-white via-cyan-50 to-white dark:from-gray-600 dark:via-slate-800 dark:to-slate-950">
              <div className="absolute inset-0 bg-[url('/assets/patterns/contact2.jpg')] md:bg-[url('/assets/patterns/contact.jpg')] bg-no-repeat bg-center bg-cover  md:bg-[length:100%_100%] opacity-20 md:opacity-30">              </div>
                <div className="container flex flex-col md:flex-row justify-center items-center mx-auto relative z-10">
                  <div>
                    <h1 className="text-5xl md:text-6xl bg-gradient-to-r from-transparent to-transparent bg-transparent bg-clip-text text-shadow-xs text-slate-600 dark:text-slate-200 orboto font-extrabold tracking-tighter mb-4">{t('heroTitle')}</h1>
                    <p className="text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto">{t('heroSubtitle')}</p>
    ...
    <div className="space-y-7">
      {contactItems.map((item) => (
        ...
        >
          ...
          {/* Text Content */}
          <div>
            <h3 className="font-semibold text-lg">{t(item.labelKey)}</h3>
            <p className="text-muted-foreground text-[16px]">{t(item.valueKey)}</p>
            {item.secondaryValueKey && (
              <p className="text-muted-foreground text-[16px]">{t(item.secondaryValueKey)}</p>
            )}
          </div>
        </div>
      ))}
      ...
      {details.latitude && details.longitude && (
        <div className="w-full mt-6 rounded-2xl overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-primary/20">
          <PropertyMap latitude={details.latitude} longitude={details.longitude} />
        </div>
      )}
    </div>
  </div>
                ...
                <div className="container z-40 mx-auto text-center relative">
                    ...
                    <div className="flex justify-center gap-4 mb-16">
                        {socialLinks.map(({ name, icon: Icon, href }) => (
                            <Button asChild key={name} variant="outline" size="icon" className="h-12 w-12 rounded-full hover:bg-primary hover:text-white transition-all duration-300 transform hover:scale-110">
                                <a href={href} aria-label={name}><Icon className="h-5 w-5" /></a>
                            </Button>
                        ))}
                    </div>

                    <div className="max-w-3xl mx-auto">
                         <h2 className="text-3xl text-shadow-xs text-shadow-black orboto font-extrabold mb-4">{t('bottomCtaTitle')}</h2>
                         <p className="text-muted-foreground mb-8">{t('bottomCtaSubtitle')}</p>
                         <div className="flex sm:flex-row gap-4 justify-center">
                            ...
                            <Button asChild size="lg" variant="outline" className='border-dashed rounded-full'>
                                <a href={`tel:${t('phone1').split(' ')[0]}`}>
                                    <Phone className="h-5 w-5 mr-2" />{t('bottomCtaCall')}
                                </a>
                            </Button>
                         </div>
                    </div>

...
import { Eye,  MailPlus, MoreHorizontal,   TrendingUp,  UserPen, UserPenIcon, UserPlus } from "lucide-react";
...
// Page component
export default async function AdminTenantsPage(props: { searchParams: Promise<SearchParams> | undefined }) {
        ...
        <div className="flex items-center gap-2 text-2xl md:text-3xl font-bold tracking-tight">
            <div className="relative bg-pink-100/80 rounded-sm shadow-lg shadow-red-500/80 dark:shadow-red-700/80 p-1">
              ...
              <span>
                ...
                <p className="text-[17px] font-light text-muted-foreground">Manage tenants, leases, and invoices all in one place.</p>
        ...
        <Table>
          <TableHeader className="bg-gray-100 dark:bg-gray-800">
            <TableRow className="bg-muted/50">
              ...
              <TableHead className="hidden md:table-cell">Active Lease</TableHead>
              <TableHead>
                               <div className="flex items-center gap-1">
          ...
          <TableBody>
            {tenants.map((tenant, i) => {
              ...
              return (
                <TableRow
                  key={tenant.id}
                  className={i % 2 === 0 ? "bg-background" : "bg-muted/30 hover:bg-muted/50"}
                >
                  {/* Tenant */}
                  <TableCell>
                    <div className="flex items-center gap-3">
                      <Avatar className="h-12 w-12">
                        <AvatarImage src={tenant.image || undefined}  alt={tenant.name?.charAt(0).toUpperCase()} />
                  ...
                  {/* Email */}
                  <TableCell className="text-muted-foreground hidden md:table-cell">{tenant.email}</TableCell>
                    ...
                    {tenant.leases?.length > 0 ? (
                      tenant.leases.map((lease) => (
                        <div key={lease.id} className="text-sm">
                          <span className="font-semibold">{lease.room.roomNumber}</span>{" "}
                          <span className="text-muted-foreground">at {lease.room.property.name}</span>
                        </div>
                      ))
                    ) : (
                  ...
                  <TableCell className="text-right gap-2 ">
                    ...
                    <Button asChild variant="outline" size="sm">
                     ...
                     <div className="hidden md:block">
                                          {/* <AssignPropertyDialog tenant={ticket} /> */}

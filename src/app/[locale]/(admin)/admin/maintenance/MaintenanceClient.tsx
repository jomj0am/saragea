...
import { Property, Room, type MaintenanceTicket } from '@prisma/client';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Badge } from "@/components/ui/badge";
import TicketStatusChanger from "@/components/admin/naintainance/TicketStatusChanger";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Wrench, Search, Filter, Calendar, Building2, Info,  UserPen, MoreHorizontal, TrendingUp, PenTool, Pin } from "lucide-react";
...
type FullTicket = MaintenanceTicket & {
  tenant: { name: string | null , image: string};
...
export default function MaintenanceClient({ initialTickets }: { initialTickets: FullTicket[] }) {
  ...
  return (
    <section className=" xl:p-8 space-y-8">
     ...
     <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <div className="flex  sm:flex-row items-stretch sm:items-center gap-3">
        ...
        <Button variant="outline" className="gap-2 dark:border-slate-700/30 rounded-full shadow-sm bg-gradient-to-br from-slate-200/20 to-slate-300 dark:from-slate-400 dark:to-slate-700">
          <Filter className="h-4 w-4 fill-emerald-100 text-emerald-400" /> Filter
        </Button>
      </div>
          ...
          <TableHeader>
            ...
            <TableHead>
                <div className="flex items-center gap-2">
                  <div className="h-8 w-12 my-2 relative flex bg-gradient-to-br from-slate-400 dark:from-slate-700 to-slate-900 dark:border-slate-700/30 border  rounded-full justify-center items-center"><UserPen className="text-pink-500 fill-pink-100"/> </div>Tenant
                </div>
</TableHead>
    ...
    Room / Property
              ...
              <TableHead>
                               <div className="flex items-center gap-1">
                                  <PenTool className="w-4 drop-shadow-sm dark:shadow-md fill-fuchsia-200 text-fuchsia-400 drop-shadow-black"/>    Title
                              </div>
              </TableHead>
              <TableHead>Vendor</TableHead>
              <TableHead>
                               <div className="flex items-center gap-1">
                                  <TrendingUp className="w-4 drop-shadow-sm dark:shadow-md fill-green-200 text-green-400 drop-shadow-black"/>    Status
                              </div>
              </TableHead>
              <TableHead >
                                <div className="flex items-center gap-2  pr-2">
                                    <Calendar className="w-4 drop-shadow-sm dark:drop-shadow-md drop-shadow-black/80 text-yellow-500 fill-yellow-100"/>Date
                                </div>
              </TableHead>
          ...
          <TableBody>
            {filteredTickets.map((ticket, i) => (
              <TableRow key={ticket.id} className={`transition-colors ${i % 2 === 0 ? "bg-background" : "bg-muted/20"} hover:bg-muted/40`}>
                <TableCell className="font-medium flex gap-2 items-center">
                    <div className="flex items-center gap-3">
                      <Avatar className="h-12 w-12">
                        <AvatarImage src={ticket.tenant.image || undefined}  alt={ticket.tenant.name?.charAt(0).toUpperCase()} />
                        <AvatarFallback>{ticket.tenant.name?.charAt(0).toUpperCase()}</AvatarFallback>
                      </Avatar>
                    </div>
                   {ticket.tenant.name}
                </TableCell>
                <TableCell >
                        ...
                        {/* <span className="text-muted-foreground">at {ticket.room.property.name}</span> */}
                </TableCell>
                <TableCell className="max-w-[20rem] truncate">{ticket.title}</TableCell>
                <TableCell>{ticket.vendor?.name ?? <span className="text-muted-foreground italic">Unassigned</span>}</TableCell>
                <TableCell>
      ...
      <div className="md:hidden space-y-4">
        {filteredTickets.map((ticket) => {
          ...
          return (
            <div key={ticket.id} className=" border shadow-md p-4 bg-white/90 dark:bg-gray-900/80">
                      ...
                      <div className=" ">
                         {ticket.tenant.name}
                          <p className="text-sm text-muted-foreground flex items-center gap-1">
                                <Calendar className="w-3 h-3" /> {formatDate(new Date(ticket.createdAt))}
                          </p>
                      </div>
                     </div>
                      <Badge variant={statusVariant(ticket.status)} className="capitalize w-max mr-2">
                          {ticket.status.replace("_", " ").toLowerCase()}
                      </Badge>
                  </div>
                  <h2 className="font-semibold mt-1 border-t pt-2 line-clamp-2 flex items-center"><Pin className='w-5 h-5 text-red-200 fill-red-500 mr-2'/>{ticket.title}</h2>
              ...
              {isOpen && (
                <div className="mt-3 space-y-2 border-t p-3 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-lg ">
                  <p className="text-sm text-muted-foreground flex items-center gap-1">
                    <Building2 className="w-4 h-4" /> Room {ticket.room.roomNumber}
                  </p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <p className="line-clamp-3 text-muted-foreground">{ticket.description ?? "No additional info"}</p>
                  </div>

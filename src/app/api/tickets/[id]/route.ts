  ...
  try {
    ...
    if (dataToUpdate.status) {
      await sendNotification({
        userId: updatedTicket.tenantId, // The tenant who owns the ticket
        title: "Maintenance Update",
        message: `Your ticket "${updatedTicket.title}" is now: ${dataToUpdate.status}`,
        type: dataToUpdate.status === "RESOLVED" ? "SUCCESS" : "INFO",

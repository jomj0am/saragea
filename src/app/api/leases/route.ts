...
export async function POST(req: Request) {
  ...
  try {
    const { tenantId, roomId, startDate, endDate, amount, paymentMethod } =
      await req.json();
    ...
    const newLease = await prisma.$transaction(async (tx) => {
      ...
      await sendNotification({
        userId: tenantId,
        title: "Welcome Home! 🏠",
        message: `You have been assigned to Room. Your lease is now active.`,
        type: "SUCCESS",
        link: "/dashboard",
        sendEmail: true,
      });
    ...
    await logAction(
      ...
      "CREATE",
      "LEASE",
      `Assigned Room ${roomId} to Tenant ${tenantId}`
    );
    ...
    return NextResponse.json(
      { message: message || "Failed to create lease" },
      { status: 500 }
    );
  }
}

...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (!validation.success) {
             return NextResponse.json({ message: 'Invalid payload format.', errors: validation.error.flatten() }, { status: 400 });
        }
        ...
        return NextResponse.json({ message: 'Failed to update settings', error: errorMessage }, { status: 500 });
    }
}
...
// ... TranslationInput component remains the same (omitted for brevity) ...
const TranslationInput = ({
  ...
  const [activeLocale, setActiveLocale] = useState<(typeof locales)[number]>(
    locales[0]
  );
...
export default function AboutPageEditor({
  ...
  const [translations, setTranslations] =
    useState<EditableTranslation[]>(initialTranslations);
  const [isSaving, setIsSaving] = useState(false);
  const { toast } = useToast();
  const router = useRouter();

  const handleMediaChange = (key: string, value: string) => {
    handleTranslationChange(key, "en", value);
  };
  ...
  const getValue = (key: string) => {
    return (
      translations.find((t) => t.key === key && t.locale === "en")?.value || ""
    );
  };
    ...
    <div className="space-y-8">
      <Accordion type="single" collapsible className="w-full space-y-4">
        ...
        <AccordionItem value="hero">
          ...
          <AccordionContent className="px-3 md:px-6 pb-6 space-y-6 mt-2">
            {/* Text Content */}
            <Card className="bg-white dark:bg-gray-900 shadow-md rounded-lg border-l-4 border-purple-500">
              <CardHeader>
            ...
            <Card className="bg-white dark:bg-gray-900 shadow-md rounded-lg border-l-4 border-blue-500">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <ImageIcon className="w-5 h-5" /> Hero Background Image
                </CardTitle>
                <CardDescription>
                  Upload a high-quality image for the main banner.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  ...
                  <MultiImageUploader
                    value={
                      getValue("AboutPage.heroImage")
                        ? [getValue("AboutPage.heroImage")]
                        : []
                    }
                    onChange={(urls) =>
                      handleMediaChange("AboutPage.heroImage", urls[0] || "")
                    }
                  />
                </div>
              </CardContent>
        ...
        <AccordionItem value="story">
          ...
          <AccordionContent className="px-3 md:px-6 pb-6 space-y-6 mt-2">
            ...
            {/* Visuals & Stats */}
            <Card className="bg-white dark:bg-gray-900 shadow-md rounded-lg border-l-4 border-teal-500">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <ImageIcon className="w-5 h-5" /> Visuals & Stats
                </CardTitle>
                <CardDescription>
                  Manage the floating images and the statistic card.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label>Main Large Image</Label>
                    <MultiImageUploader
                      value={
                        getValue("AboutPage.storyMainImage")
                          ? [getValue("AboutPage.storyMainImage")]
                          : []
                      }
                      onChange={(urls) =>
                        handleMediaChange(
                          "AboutPage.storyMainImage",
                          urls[0] || ""
                        )
                      }
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <MultiImageUploader
                      value={
                        getValue("AboutPage.storySecondaryImage")
                          ? [getValue("AboutPage.storySecondaryImage")]
                          : []
                      }
                      onChange={(urls) =>
                        handleMediaChange(
                          "AboutPage.storySecondaryImage",
                          urls[0] || ""
                        )
                      }
                    />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t">
                  <div className="space-y-2">
                    <Label className="flex items-center gap-2">
                      <TrendingUp className="w-4 h-4" /> Stat Value
                    </Label>
                    <TranslationInput
                      translations={translations}
                      translationKey="AboutPage.storyStatValue"
                      onChange={handleTranslationChange}
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <TranslationInput
                      translations={translations}
                      translationKey="AboutPage.storyStatLabel"
                      onChange={handleTranslationChange}
                    />
                  </div>
                </div>
        ...
        <AccordionItem value="how">
          <AccordionTrigger className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-yellow-200 to-orange-200 dark:from-yellow-800 dark:to-orange-700 rounded-lg shadow-md hover:shadow-lg">
            <Layers className="h-5 w-5 text-yellow-600 dark:text-yellow-300" />{" "}
            How It Works
          </AccordionTrigger>
            ...
            <Card className="bg-white dark:bg-gray-900 shadow-md rounded-lg border-l-4 border-indigo-500">
              <CardContent className="space-y-4 pt-6">
                ...
                <TranslationInput
                  ...
                  component={Textarea}
                />
                <Label className="font-semibold mt-4">Value 3: Comfort</Label>
                <TranslationInput
                  translations={translations}
                  translationKey="AboutPage.value3Desc"
                  onChange={handleTranslationChange}
                  component={Textarea}
                />
              </CardContent>

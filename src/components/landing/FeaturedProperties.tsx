...
export default function FeaturedProperties({
  ...
  return (
    <section className="relative py-24 px-4 md:px-0 bg-gradient-to-br from-secondary/80 via-white to-secondary dark:from-slate-950 dark:via-slate-600/30  dark:to-slate-950">
                 
      <div className="absolute inset-0 bg-[url('/assets/patterns/grid.jpg')] opacity-10 dark:opacity-5 pointer-events-none" />
      <div className="container mx-auto relative flex flex-col lg:flex-row gap-10">
        {/* Left rail (desktop) */}
        <div className="hidden lg:flex flex-col justify-center gap-4">
          <ThumbRail thumbs={showThumbs} active={active} setSlide={setSlide} vertical />
        </div>

        <div className="flex-1">
          <div className="text-center mb-14">
            <h2 className="text-4xl md:text-5xl orboto font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-primary to-purple-500 drop-shadow-md">
          ...
          <div className="mt-6 flex items-center justify-center lg:hidden">
            <div ref={thumbRowRef} className="flex w-full items-center justify-center scrollbar-hide">
              <ThumbRail thumbs={showThumbs} active={active} setSlide={setSlide} vertical={false} />
            </div>
          </div>
        </div>

        <Link
          className="absolute -bottom-12 right-0 flex underline text-blue-500 text-shadow-xs text-shadow-black"
          href="properties"
        >
...
function ThumbRail({
  ...
  return (
    <div
      className={clsx(
        'flex',
        vertical ? 'flex-col gap-3' : 'flex-row gap-2 overflow-hidden px-1 py-1'
      )}
    >
      {thumbs.map((prop, i) =>
        prop.images && prop.images.length > 0 ? (
          ...
          <div
            key={prop.id}
            className="h-20 md:w-28 w-22 flex-shrink-0 rounded-xl bg-gray-300/20 border border-dashed border-gray-400 animate-pulse"
          />
        )
      )}
    </div>
